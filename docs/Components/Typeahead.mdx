...
<Playground>
    <StatefulComponent initialState={{suggestions: []}}>
    {
        ({state, setState}) => {
            ...
            const renderInputValue = ({name}) => (name);
            return <Typeahead renderSuggestion={renderSuggestion} placeholder="Search by team, artist, event or venue" onChange={e => getSuggestion(e)} onSelect={onSelect} suggestions={state.suggestions} renderInputValue={renderInputValue}/>
        }
    }
    </StatefulComponent>
